...
void function ShowNewHighscoreMessage( string playerName, float playerTime, bool isWorldRecord )
{
	...
	RuiSetString(
		file.resetHintRUI,
		"descriptionText",
		Localize( isWorldRecord ? "#NEW_WORLD_HIGHSCORE_TEXT" : "#NEW_LOCAL_HIGHSCORE_TEXT", playerName, format( "%.2f", playerTime ) )
	)
...
void function SpawnStartLine()
{
	if ( file.startLineTopology == null )
		file.startLineTopology =
			CreateTopology( file.startLine.origin, file.startLine.angles, file.startLine.dimensions[ 0 ].tofloat(), file.startLine.dimensions[ 1 ].tofloat() )
